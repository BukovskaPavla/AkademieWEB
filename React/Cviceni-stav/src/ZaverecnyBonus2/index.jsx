...
const Kasicka = ({ pocatecniStav }) => {
	const [suma, setSuma] = useState(pocatecniStav)
	return (
		<div className="kasicka">
			<h3>
				<img src={penizeUrl} width={24} height={24} alt="" /> Peníze v kasičce:{suma} Kč
			</h3>
			<button onClick={()=>setSuma(suma+5)}>přihodit pětikorunu</button> <button onClick={()=>setSuma(0)}>vysypat</button>
		</div>
	)
}
